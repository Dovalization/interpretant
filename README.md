...
The distributional hypothesis underlying Word2Vec — "You shall know a word by the company it keeps" (Firth, 1957) — converges with Wittgenstein's observation that, for a large class of cases, meaning is use. A word vector encodes not what a word _is_ but how it _behaves_ in relation to other words across a corpus. In Peircean terms, each vector encodes the aggregate of **dynamic interpretants** — Peirce defines the dynamic interpretant as the actual effect a sign produces on a given mind at a given moment; aggregated across a community and a decade, the corpus statistics capture the collective shape of those effects — as opposed to the immediate interpretant (the sign's interpretable meaning as it stands, prior to any specific context: the schema of understanding a sign carries in abstraction from its actual effects) or the final interpretant (the ideal interpretive effect that would be reached at the convergent end of unlimited inquiry). Corpus statistics capture the aggregate of actual interpretive events — which is precisely the dynamic variety.
